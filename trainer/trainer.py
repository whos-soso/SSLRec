...
class AdaGCL2Trainer(Trainer):
    def __init__(self, data_handler, logger):
        from models.general_cf.adagcl2 import VGAE, DenoiseNet
        super(AdaGCL2Trainer, self).__init__(data_handler, logger)
        self.generator_1 = VGAE().cuda()
        self.generator_2 = DenoiseNet().cuda()
...
class AdaGCLwogdTrainer(Trainer): 
    ...
    def __init__(self, data_handler, logger):
        ...
        super(AdaGCLwogdTrainer, self).__init__(data_handler, logger)
        self.generator_1 = VGAE().cuda()
        #self.generator_2 = DenoiseNet().cuda()
    ...
    def train_epoch(self, model, epoch_idx):
        ...
        for _, tem in tqdm(enumerate(train_dataloader), desc='Training Recommender', total=len(train_dataloader)):
            ...
            loss_dict = {**loss_dict_cl, **loss_dict_ib, **loss_dict_main, **loss_dict_vgae}
             # record loss
            for loss_name in loss_dict:
                _loss_val = float(loss_dict[loss_name]) / len(train_dataloader)
                if loss_name not in loss_log_dict:
...
class AdaGCLKANTrainer(Trainer):
    def __init__(self, data_handler, logger):
        from models.general_cf.adagcl_kan import VGAE, DenoiseNet
        super(AdaGCLKANTrainer, self).__init__(data_handler, logger)
        self.generator_1 = VGAE().cuda()
        self.generator_2 = DenoiseNet().cuda()
